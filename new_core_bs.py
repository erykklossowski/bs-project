...
aFRR-DOWN valuation (capacity-only) with:
 - explicit portfolio-level budget cap,
 - ex-ante Black–Scholes-style put option valuation,
...
Examples:
  ...
  python affr_down_refactored.py \
      ...
      --S-cap-pln-per-mwh 400 --portfolio-mw 50 --total-portfolio-cap-pln 5000000 --out-prefix run_K400
  ...
  python affr_down_refactored.py \
      ...
      --S-cap-pln-per-mwh 400 --portfolio-mw 50 --total-portfolio-cap-pln 5000000 --ridge 1.0 --mode both --out-prefix wind_afrrd_K400
...
def bs_put_lognormal(mu: np.ndarray, sigma: float, K: float) -> np.ndarray:
    """
    Calculates the expected value of a put option payoff E[max(K - S, 0)]
    for a lognormally distributed asset price S.
    The formula is P = K * N(-d2) - S * N(-d1), simplified for zero interest rates.
    This models the BSP's revenue from the TSO exercising the option.
    """
    K = max(K, 1e-12)
    if sigma <= 1e-12:
        s0 = np.exp(mu)
        return np.maximum(K - s0, 0.0)
    d1 = (mu + 0.5 * sigma**2 - math.log(K)) / sigma
    d2 = d1 - sigma
    term1 = K * norm.cdf(-d2)
    term2 = np.exp(mu + 0.5 * sigma**2) * norm.cdf(-d1)
    return np.maximum(term1 - term2, 0.0)

def realized_put_payoff(price: np.ndarray, K: float) -> np.ndarray:
    """Calculates the realized payoff for a put option: max(K - price, 0)."""
    return np.maximum(K - price, 0.0)
...
def run_pipeline(
    ...
    S_cap_pln_per_mwh: float, portfolio_mw: float, total_portfolio_cap_pln: float, ridge: float, mode: str, out_prefix: str
):
    ...
    # 6) Ex-ante Black–Scholes (μ_t from predictors, σ global)
    results = {}
    if mode in ("exante","both"):
        ...
        # Calculate expected put option value. This represents the expected energy payment to the BSP
        # if the price falls below the strike (S_cap).
        put_val_per_mwh = bs_put_lognormal(mu_t, sigma, S_cap_pln_per_mwh)
        df["exante_put_val_per_mwh"] = put_val_per_mwh
        
        # Total baseline value for the portfolio (uncapped)
        exante_total_baseline_pln = (put_val_per_mwh * portfolio_mw * df["Hours_in_interval"]).sum()
        
        # Apply the fixed total portfolio cap
        exante_total_capped_pln = min(exante_total_baseline_pln, total_portfolio_cap_pln)
        
        results.update({
            "exante_sigma": float(sigma),
            "exante_beta": {f"b{i}": float(b) for i,b in enumerate(beta)},
            "exante_total_baseline_pln": exante_total_baseline_pln,
            "exante_total_capped_pln": exante_total_capped_pln,
        })
    ...
    if mode in ("expost","both"):
        ...
        # Calculate realized put payoff. This is the realized energy payment.
        realized_put_payoff_val = realized_put_payoff(price, S_cap_pln_per_mwh)
        df["expost_put_val_per_mwh"] = realized_put_payoff_val

        # Total realized value for the portfolio (uncapped)
        expost_total_baseline_pln = (realized_put_payoff_val * portfolio_mw * df["Hours_in_interval"]).sum()
        
        # Apply the fixed total portfolio cap
        expost_total_capped_pln = min(expost_total_baseline_pln, total_portfolio_cap_pln)

        results.update({
            "expost_total_baseline_pln": expost_total_baseline_pln,
            "expost_total_capped_pln": expost_total_capped_pln,
        })
    ...
    # Per-interval CSV
    df_out = df[[
        "dt","Hours_in_interval","price_pln_per_mw_h","mw_procured",
        *([ "exante_put_val_per_mwh" ] if "exante_put_val_per_mwh" in df.columns else []),
        *([ "expost_put_val_per_mwh" ] if "expost_put_val_per_mwh" in df.columns else []),
    ]]
    ...
    with open(f"{out_prefix}_summary.json","w",encoding="utf-8") as f:
        json.dump({
            "S_cap_pln_per_mwh": S_cap_pln_per_mwh,
            "portfolio_mw": portfolio_mw,
            "total_portfolio_cap_pln": total_portfolio_cap_pln,
    ...
    print("[ok] saved:",
          f"{out_prefix}_intervals.csv, {out_prefix}_summary.json")
...
def main():
    ...
    # Valuation
    ap.add_argument("--S-cap-pln-per-mwh", type=float, default=400.0, help="Strike price [PLN/MW-h]. Payoff is max(K-S, 0).")
    ap.add_argument("--portfolio-mw", type=float, default=1.0, help="Portfolio size in MW")
    ap.add_argument("--total-portfolio-cap-pln", type=float, required=True, help="Total fixed budget cap for the portfolio over the analysis period [PLN].")
    ...
    run_pipeline(
        ...
        S_cap_pln_per_mwh=args.S_cap_pln_per_mwh, portfolio_mw=args.portfolio_mw, total_portfolio_cap_pln=args.total_portfolio_cap_pln, ridge=args.ridge, mode=args.mode, out_prefix=args.out_prefix
    )
